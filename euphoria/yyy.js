...
//---------------------------------------КАСТ-------------------------------------------
	...
	$('.button-right-cast').on('click', function(){
		var trlBRC = $('.cast-menu').css('left');
		dlinaRigntStr = - trlBRC.length;
		trlBRC2 = parseInt(trlBRC.slice(dlinaRigntStr, -2));
		if (trlBRC2 <= -2400) {
			$('.cast-menu').css("left", "0px");
		} else {
			$('.cast-menu').css("left", "-=500");
		}
	});


	$('.button-left-cast').on('click', function(){
		var trlBLC = $('.cast-menu').css('left');
		dlinaLeftStr = - trlBLC.length;
		trlBLC2 = parseInt(trlBLC.slice(dlinaLeftStr, -2));
		if (trlBLC2 >= 0) {
			$('.cast-menu').css("left", "0px");
		} else {
			$('.cast-menu').css("left", "+=500");
